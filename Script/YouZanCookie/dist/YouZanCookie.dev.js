...
var $ = new Env('有赞-Cookie版本签到');
var YouZanCookie = ($.isNode() ? JSON.parse(process.env.YouZanCookie) : $.getjson("YouZanCookie")) || [];
var activityWeapp = [{
  "18201": "小罐茶官方旗舰店"
}];
var notice = '';
      ...
      switch (_context3.prev = _context3.next) {
        case 0:
          ...
          _loop = function _loop() {
            var item, checkinId, name, _iteratorNormalCompletion2, _didIteratorError2, _iteratorError2, _iterator2, _step2, data, id, kdtId, cookie, checkin;
              ...
              while (1) {
                switch (_context2.prev = _context2.next) {
                  case 0:
                    ...
                    name = checkinId; // 获取微信小程序名称

                    if (activityWeapp.find(function (item) {
                      return checkinId in item;
                    })) {
                      name = activityWeapp.find(function (item) {
                        return checkinId in item;
                      })[checkinId];
                    }
                  ...
                  case 11:
                    ...
                    data = _step2.value;
                    id = data.id, kdtId = data.kdtId, cookie = data.cookie;
                    console.log("\u7528\u6237\uFF1A".concat(id, "\u5F00\u59CB\u7B7E\u5230"));
                    _context2.next = 17;
                    return regeneratorRuntime.awrap(commonGet("checkinId=".concat(checkinId, "&kdt_id=").concat(kdtId), cookie));
          ...
          _iterator = YouZanCookie[Symbol.iterator]();
...
function getCookie() {
  var cookie, urlStr, urlQuerys, paramsArr, i, len, arr, checkinId, kdtId, name, yzLogWeappDataString, yzLogWeappData, newData, userCookie, existingIndex, index;
    ...
    while (1) {
      switch (_context4.prev = _context4.next) {
        case 0:
          ...
          $.msg($request.url, "".concat(checkinId), '获取额外数据失败');
        ...
        case 5:
          ...
          checkinId = urlQuerys.checkinId;
          kdtId = urlQuerys.kdt_id; // 获取微信小程序名称

          name = checkinId; // 使用 find 方法找到包含目标键的对象
          ...
          console.log($.name, "$hook ".concat(checkinId, " \u7B7E\u5230\uFF1A").concat(name));
          $.msg($.name, "hook ".concat(checkinId, " \u7B7E\u5230\uFF1A").concat(name));
          yzLogWeappDataString = getCookieValue(cookie, 'yz_log_weapp_data'); // 解析为 JSON 对象
          ...
          userCookie = {
            "id": yzLogWeappData.user.uuid,
            "cookie": cookie,
            "kdtId": kdtId
          };
          console.log("".concat(name, " usercookie "), userCookie); // 判断是否已缓存该小程序 token

          existingIndex = YouZanCookie.findIndex(function (e) {
            return e.checkinId == checkinId;
          });
          ...
          // 已缓存该小程序    
          // 判断是否已缓存该用户
          index = YouZanCookie[existingIndex].data.findIndex(function (e) {
            return e.id == userCookie.id;
          });
          ...
          console.log("".concat(name, " \u91CD\u590D\u83B7\u53D6 cookie "), cookie);
          $.msg($.name, "".concat(name), "\uD83C\uDF89\u7528\u6237 ".concat(userCookie.id, " \u91CD\u590D\u83B7\u53D6 cookie!"));
          return _context4.abrupt("return");
        ...
        case 30:
          ...
          $.msg($.name, "".concat(name), "\uD83C\uDF89\u7528\u6237".concat(userCookie.id, "\u66F4\u65B0token\u6210\u529F!"));
        ...
        case 35:
          ...
          console.log(JSON.stringify(userCookie));
          $.msg($.name, "".concat(name), "\uD83C\uDF89\u65B0\u589E\u7528\u6237".concat(userCookie.id, "\u6210\u529F!"));
        ...
        case 40:
          // 未缓存该小程序
          console.log($.name, "".concat(name), "发现新的签到活动");
          $.msg($.name, "".concat(name), "\uD83C\uDF89\u53D1\u73B0\u65B0\u7684\u7B7E\u5230\u6D3B\u52A8!");
        ...
        case 46:
          $.setjson(YouZanCookie, "YouZanCookie"); // 执行签到测试
